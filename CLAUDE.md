...
- Exact re-uploads (same SHA-256 within a business) return the existing document with `duplicate: true` instead of storing a second copy.
- **Extraction queue** (`apps/api/src/extractions`): after a successful upload, `DocumentsService` adds a BullMQ job (`extraction` queue, job id = document id, so re-adding is a no-op); jobs carry only ids. `ExtractionProcessor` runs in the API process (concurrency 2) and calls `ExtractionsService.run()`. Retryable failures are rethrown for job-level retries (3 attempts, 30s exponential backoff); permanent failures, 404s and 409s (already claimed) complete the job. `ExtractionRecoveryService` runs on boot: stale PROCESSING (>15 min) → QUEUED, then re-enqueues every QUEUED document. `EXTRACTION_WORKER=off` disables the worker and recovery in a process; e2e tests default to off and get a unique `QUEUE_PREFIX` per file (`test/setup-e2e.ts`), so tests never trigger paid extractions. The boot sweep spans all businesses, so e2e tests also set `EXTRACTION_RECOVERY=off` and call the business-scoped `recover({ businessId })` instead: the e2e database is the dev database.
- **Review & corrections** (`apps/api/src/reviews`): `POST /documents/:id/review` with `{ corrections: [{ fieldId, value }] }` validates every value into canonical form (`field-values.ts`; empty = not on document), skips unchanged values, writes one `Correction` row plus a `field.corrected` audit entry per change, and moves REVIEW → DONE, all in one interactive transaction (all or nothing; the status move is conditional). DONE documents can be re-edited. Fields must belong to the document's latest run. The current value of a field is its latest correction, else the model value (`latest-correction.ts`); `GET /documents/:id` returns `value` (current), `aiValue` (model) and `correction` (who/when).
- **Review UI** (`apps/web/src/app/(app)/documents/[id]/`): `ReviewForm` (client) edits fields in local state and submits only changed ones, as JSON in a hidden input, via the `submitReview` server action, which calls `refresh()` from `next/cache` on success. The page keys the form on status + latest correction time so it remounts with fresh values after a save. API 400s come back as `path: message` and are shown under the matching input until that value is edited. Virus scanning is not implemented; the hook point is marked in `DocumentsService.upload`.
